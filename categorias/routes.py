...
def get_categorias():
    result = db.session.execute(db.text("SELECT * FROM categorias"))
    categorias = []
    for row in result:
        categorias.append({
            'id': row.id,
...
def create_categoria():
    ...
    query = db.text("""
        INSERT INTO categorias (nombre, descripcion, estado, creador, creacion, modificador, modificacion)
    ...
    categoria = db.session.execute(
        db.text("SELECT * FROM categorias WHERE id = :id"), 
        {'id': categoria_id}
    ).fetchone()
...
@categorias_bp.route('/api/categorias/<int:id>', methods=['GET'])
...
def delete_categoria(id):
    result = db.session.execute(
        db.text("DELETE FROM categorias WHERE id = :id"), 
        {'id': id}
    )
